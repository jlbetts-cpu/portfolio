#!/usr/bin/env python3
"""Deterministically composite and encode the Apollo time-of-day thumbnails."""
...
OUT = ROOT / "images/cs/variants/time/apollo"
SOURCES = OUT / "sources"
MASTER = ROOT / "images/cs/masters/apollo-mockups.png"
STATES = ("pre-dawn", "sunrise", "daytime", "dusk", "sunset", "night")
CANVAS_SIZE = (2400, 1784)
MOBILE_SIZE = (1200, 892)
MOCKUP_SIZE = (1800, 939)
MOCKUP_POSITION = (300, 423)
...
def main() -> None:
    ...
    with Image.open(MASTER) as source_master:
        if source_master.size != (3360, 1752) or source_master.mode != "RGBA":
            raise RuntimeError(f"Unexpected Apollo master: {source_master.size} {source_master.mode}")
        # This is the only foreground transformation: one deterministic geometric downscale.
        resized_mockup = source_master.resize(MOCKUP_SIZE, Image.Resampling.LANCZOS)

    resized_mockup.save(SOURCES / "mockup-resized-1800x939.png", optimize=True)
    ...
    with tempfile.TemporaryDirectory(prefix="apollo-variants-") as temp_dir:
        temp = Path(temp_dir)
        for state in STATES:
            ...
            metrics["states"][state] = {
                ...
                "plate_dimensions": list(Image.open(plate_path).size),
                "desktop": {
