#!/usr/bin/env python3
"""Deterministically composite and encode the Strata time-of-day thumbnails."""
...
SOURCES = OUT / "sources"
MASTER = ROOT / "images/cs/masters/strata-mockups.png"
STATES = ("pre-dawn", "sunrise", "daytime", "dusk", "sunset", "night")
CANVAS_SIZE = (2400, 1784)
MOBILE_SIZE = (1200, 892)
MOCKUP_SIZE = (1800, 948)
MOCKUP_POSITION = (300, 420)
...
def main() -> None:
    ...
    with Image.open(MASTER) as source_master:
        if source_master.size != (3360, 1770) or source_master.mode != "RGBA":
            raise RuntimeError(f"Unexpected Strata master: {source_master.size} {source_master.mode}")
    ...
    resized_mockup.save(SOURCES / "mockup-resized-1800x948.png", optimize=True)
    ...
    with tempfile.TemporaryDirectory(prefix="strata-variants-") as temp_dir:
        temp = Path(temp_dir)
        for state in STATES:
