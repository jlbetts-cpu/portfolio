...
No coloured text. No hue as a border. No coloured dot before a label — that was removed.
...
0. **The header — two tiles.** The wordmark is a tile; the links, the theme toggle and Subscribe are gathered into a second. Both sit on the page's own gutter with the same `--r-md` corner as every panel under them, `--grid-gap` from the top. It is not a bar and not a glass strip. There is no Home link — the logo is the way home — and no Gallery link: the gallery *is* the hero, so a nav item pointing at the top of the page was pointing at itself. **About** and **Contact**.
1. **Hero: two panels.** The copy on a **cream panel**, left five columns; the right seven a **colour panel** holding a bento of fifteen photographs in three columns, looping vertically with the flow, adjacent columns in opposite directions at three speeds (`--speed` per column). **The panel's own rounded edge is the crop** — there is no soft mask. Each column carries its contents twice and `[data-mid]` marks the loop length; every tile has an intrinsic ratio, so that length is stable before the images load. The panel needs an explicit `height`: `overflow: hidden` does not constrain a box sizing to its own content, and a column of ten tiles once made the row 3197px tall. The hero no longer claims `100svh`: the card row has to break the fold, or the field reads as two pages rather than one.
2. **The four cards.** One row, four identical objects, three columns each: a **head that is the card's hue** carrying two chips, the title and a two-line summary; the photograph below it; one control. Two cards say what Developmental Improvisation is, one says what it asks of a student, one says who Linda is. **The photograph slips `--slip` 20px up out of its box and over the colour** — the one place on the page a panel's contents cross an edge, and small on purpose: 20px reads as deliberate, 60px reads as broken. The whole card is the trigger and the "Read more" pill is the keyboard route; both open **the reader**, a dialog whose head takes that card's hue so the panel clicked and the panel opened are plainly the same object. The full copy lives in the card (`.brief__full`) and is hidden only when there is script to open a reader.
...
4. **Testimonials — the field resumes.** Three cards, four columns each, each a hue: a large quote mark at 26% of the card's ink, the quote, and the person in a **nested card that overhangs the bottom-left corner**. The depth comes from the nesting, not from a shadow. The row carries 30px of bottom padding because that overhang has to land somewhere — without it the three sub-cards sat on the closing field's edge. There is no section label: three quotes with names under them do not need to be told they are testimonials.
5. **The closing field** — the last panel in the field, in the same container as every row above it (it used to size itself, which put its edges 36px outside the column at 1920). The brand's own line at display scale, then the sign-up, the two contact links and the copyright, in one sky field.
