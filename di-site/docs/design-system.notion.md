...
## Arrival
Every section enters with intent rather than one flat 12px fade: the cards bring the photograph first and the text a beat later (90ms), the band's picture settles from a 1.06 scale, the ring assembles one circle at a time, the testimonials stagger, the closing field rises. One `IntersectionObserver` drives all of it through `.reveal` and `.reveal--parts`. **Never observe a zero-area element** — the ring's orbit is a 0×0 positioning origin, and an element with no area can never satisfy a `threshold: 0.2`, so the ring simply never appeared; the *stage* is what is observed.
...
> The closing field — opens with the brand's own line at display scale, then the sign-up, the two contact links and the copyright, in one sky field inset by the gutter. It is the last thing anyone sees and it was a form and two links.
...
## Components
`.btn` (primary/secondary/ghost/compact, 46px, `--r-sm`) · `.arrow` (44px circle) · `.theme` (the toggle; the choice is applied before first paint by an inline script, so there is no flash) · `.card` / `.card--solid` / `.card--line` · `.photo` (+ `--4x5`, `--1x1`, `--circle`) · `.label` (12px caps, no dot) · `.field` / `.input` · `.nav` (the wordmark on the page and everything else gathered into one panel with the same superellipse corner as every other surface; no Home link — the logo is the way home) · `.sheet` · `.dialog` · `.lightbox`. Each is on `styleguide.html` in both themes.
