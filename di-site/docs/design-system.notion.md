...
0. **The header — no container at all.** The wordmark on the left, the links, the theme toggle and Subscribe on the right, sitting directly on the page at the field's gutter. No bar, no glass, no tiles: Jayden asked for it clean and minimal, and the panel it used to sit in was one more surface competing with the field below it. **It scrolls away with the page** — bare type over a photograph is unreadable, and the alternative to putting a surface back under it is not needing one; every action it carries (Subscribe, Contact) is reachable again at the foot of the page. There is no Home link — the logo is the way home — and no Gallery link: the gallery *is* the hero, so that item pointed at the top of the page from the top of the page. **About** and **Contact**.
