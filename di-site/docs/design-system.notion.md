...
4. **Proof, then testimonials.** Three figures at the head of the row: the page said what the method is and who made it, and nothing at all about whether it has ever been used. One wide panel, three numbers at `clamp(2.25rem, 4.6vw, 4.5rem)` — the biggest type below the hero, because a number is only proof if you can read it from across the room — with the label under each in caps. **The figures are placeholders** (`50+` is real, from Linda's own copy; the zeros are the shape of a number, not a claim) and must carry her real counts before this is ever published. Then three quiet cards, four columns each: a large quote mark at 16% of the card's ink, the quote, and the person in a **nested card that overhangs the bottom-left corner** — and the hue is on that chip, not on the card. The depth comes from the nesting, not from a shadow. **The quote is bottom-aligned against that chip** while the mark stays pinned to the top: three quotes are never the same length, and top-aligning both left the shortest card carrying all the slack between its last line and the chip. Pinned at both ends the row gets two straight lines instead of one, and the slack collects under the mark where it reads as air. The row carries 30px of bottom padding because that overhang has to land somewhere. There is no section label: three quotes with names under them do not need to be told they are testimonials.
...
## Components
`.btn` (primary/secondary/ghost/compact, 46px, `--r-sm`) · `.arrow` (44px circle) · `.chip` (the smallest tile: a label in a pill on the hairline) · `.theme` (the toggle; the choice is applied before first paint by an inline script, so there is no flash) · `.card` / `.card--solid` / `.card--line` · `.brief` (the four cards) · `.proof` (the three figures) · `.photo` (+ `--4x5`, `--1x1`, `--circle`) · `.label` (12px caps, no dot) · `.field` / `.input` · `.nav` (no container; it scrolls away) · `.sheet` · `.dialog` / `.reader` · `.lightbox`. Each is on `styleguide.html` in both themes.
