...
def stackcard(num,acc,title):
    return f'<article class="stack__card card" data-accent="{acc}" style="position:static;min-height:0;grid-template-columns:1fr"><div><span class="stack__num">({num})</span><h3 class="stack__title" style="font-size:var(--fs-h3)">{title}</h3><p class="t-body">A white card, the title in the serif, the chapter number. On the home page two photographs sit on a stage at the right by one rule: the big rounded square at the outer bottom corner, bleeding past the edge of the card, the small circle over its inner top corner; even cards face the other way.</p></div></article>'
...
<h3>The band</h3>
<p class="t-body">The seven hues, each at 44% over white, in wheel order on a strip five page-widths wide so two or three span the page at a time, blurred 56px at 70%, drifting left one pass every 72s (still under reduced motion), masked so it fades into the ground by 360px, with 6% film grain so it reads as light. The header sits on it with the colour logo; the title sinks into the fade. It returns at the foot of the page, fading up under the footer, so the page ends the way it began.</p>
...
<p class="t-body">Two eased radial gradients: the hue at the foot's right, at 62% at most, mixed toward white in oklab over seven stops so the falloff has no edge; its neighbour, fainter, at the left. Stops end in transparent white, never <code>transparent</code>, which interpolates through black and draws a grey seam. On the testimonials, the newsletter and the popup only.</p>
<div class="swatches" style="grid-template-columns:repeat(auto-fill,minmax(260px,1fr))">{blooms}</div>
<h2 class="t-h2" id="type">Type</h2>
<p class="t-body">Two faces. Instrument Serif, regular only, for the display and the section titles (the hero, the stacked cards, the quote, the newsletter, the footer's line). Plus Jakarta Sans, 400 and 600, for everything else. Tracking tightens as size grows; leading loosens as it shrinks. Measures are in <code>em</code>, never <code>ch</code>.</p>
<div class="trow"><div class="tmeta"><b>display</b><br><span>hero title · Instrument Serif 400</span><br><code>.t-display</code></div><div class="t-display" style="max-width:none">New tools for cognitive development</div></div>
<div class="trow"><div class="tmeta"><b>h2</b><br><span>section titles · Instrument Serif 400</span><br><code>.t-h2</code></div><div class="t-h2" style="max-width:none">Safe, educational, and thrilling</div></div>
...
<div class="row"><div class="card" style="flex:1 1 240px;min-height:200px"><p class="t-h3">Card</p><p class="t-body">White on the ground, hairline, --r-lg. The stacked cards.</p></div><div class="card card--bloom" data-accent="orange" style="flex:1 1 240px;min-height:200px"><p class="t-h3">Bloom</p><p class="t-body">The testimonials, the newsletter, the popup.</p></div></div>
...
<p class="t-body">See <a href="index.html" style="text-decoration:underline">index.html</a>: the header sits on the band, transparent, and becomes glass with a hairline once scrolled, with the colour logo and the wordmark on the left, the links and Subscribe on the right. The footer is the colour logo, the tagline in the serif, © and two link columns, over the band's return.</p>
<div class="demo" style="position:relative;overflow:hidden;padding:0;height:120px"><div class="aurora" aria-hidden="true" style="height:120px;-webkit-mask-image:none;mask-image:none"><div class="aurora__band"></div><div class="aurora__grain"></div></div><div class="nav__brand" style="position:relative;padding:var(--sp-10) var(--sp-6);height:auto">{logo}<span class="word">Developmental Improvisation</span></div></div>
</div></main>
