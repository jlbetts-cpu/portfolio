...
def stackcard(num,acc,title):
    return f'<article class="stack__card card card--hover" data-accent="{acc}" data-surface="hover" style="position:static;min-height:0;grid-template-columns:1fr"><div><span class="stack__num">({num})</span><h3 class="stack__title" style="font-size:var(--fs-h3)">{title}</h3><p class="t-body">White at rest; under the pointer the card takes its hue and ink, ink-2 and ink-3 flip (white on violet).</p><div class="stack__extra"><div class="chips"><span class="chip">chip</span></div></div></div></article>'
...
<svg xmlns="http://www.w3.org/2000/svg" style="display:none" aria-hidden="true"><symbol id="star" viewBox="489.5 285 57.8 66">{star}</symbol><symbol id="mark" viewBox="0 0 787 842">{whitemark}</symbol><symbol id="arc" viewBox="0 0 100 100"><path d="M 14 34 A 40 40 0 0 0 86 34"/></symbol><symbol id="ring" viewBox="0 0 100 100"><circle cx="50" cy="50" r="40"/></symbol></svg>
{sprite}
...
<nav class="sg__index" aria-label="Style guide"><a href="#colour">Colour</a><a href="#type">Type</a><a href="#space">Space</a><a href="#radius">Radius</a><a href="#motion">Motion</a><a href="#buttons">Buttons</a><a href="#chips">Chips &amp; stars</a><a href="#cards">Cards</a><a href="#photos">Photos</a><a href="#orbit">Photographs in motion</a><a href="#stack">Stacked cards</a><a href="#pile">Pile</a><a href="#fields">Fields</a><a href="#nav">Header &amp; footer</a></nav>
<div>
<h2 class="t-h2" id="colour">Colour</h2>
<p class="t-body">Two themes from one token sheet. Light is the default: a warm off-white ground and a warm black ink; the header is that same black in both themes. Dark swaps the ground and ink. Colour is spent sparingly: the photographs, the logo, the three shapes, the newsletter card, and the cards' hue under the pointer. The seven logo hues take dark ink on all but violet; on the light ground the small marks (stars, initials, the pager) use a deeper tone of the same hue so a 16px star still reads. Use the toggle at bottom right to see both.</p>
...
<div class="row"><div class="card" style="flex:1 1 240px"><p class="t-h3">Card</p><p class="t-body">White on the ground, hairline, --r-lg.</p></div><div class="card card--hover" data-accent="green" data-surface="hover" style="flex:1 1 240px"><p class="t-h3">Hover card</p><p class="t-body">White at rest, its hue under the pointer. The stack and the testimonials.</p></div><div class="card card--accent" data-accent="orange" data-surface="accent" style="flex:1 1 240px"><p class="t-h3">Accent card</p><p class="t-body">Full-strength hue, --r-xl. The newsletter, the one permanent colour surface.</p></div></div>
...
<h3>Shapes</h3>
<p class="t-body">Three marks drawn from the logo, one hue each, moved by the scroll part of the flow so they are still at rest: the star (spins slowly), the arc (spins), the ring (floats). Three placements on the home page: the hero, the testimonials, the newsletter.</p>
<div class="demo shape-row"><svg class="shape shape--star" style="--size:40px;--c:var(--c-yellow)" aria-hidden="true"><use href="#star"/></svg><svg class="shape shape--arc" style="--size:96px;--c:var(--c-sky)" aria-hidden="true"><use href="#arc"/></svg><svg class="shape shape--ring" style="--size:72px;--c:var(--c-pink)" aria-hidden="true"><use href="#ring"/></svg></div>
