// Gate: under reduced motion the arch holds still, nothing animates after 300ms and reveals still reach opacity 1;
// without it the arch drifts, a hovered photograph stops it, the stack scales the covered card, and the header's ring turns with the scroll.
...
const r1 = await pg.evaluate(async () => {
  ...
  const a0 = window.__di.flow.angle; await new Promise(r => setTimeout(r, 500)); const still = Math.abs(window.__di.flow.angle - a0) < 0.01;
...
const r2 = await pg.evaluate(async () => {
  ...
  const ring = document.querySelector('.nav .logo__ring'); const t0 = ring.style.transform;
  scrollBy(0, 400); await new Promise(r => setTimeout(r, 600));
  const ringTurns = ring.style.transform !== t0;
  ...
  return { drifts, ringTurns, stackScales };
});
report('motion (full)', r2.drifts && r2.ringTurns && r2.stackScales, JSON.stringify(r2));
