...
for (const [w, h] of [[1440, 900], [1024, 768], [390, 844], [320, 640]]) {
  ...
  const r = await pg.evaluate(() => {
    const els = [...document.querySelectorAll('a, button, input, [role=button]')].filter(e => !e.closest('p') && !e.closest('dialog:not([open])') && e.offsetParent !== null && getComputedStyle(e).display !== 'none' && !e.classList.contains('skip'));
    let min = 1e9, worst = '';
