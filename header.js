...
/* ── THE RAIL's behaviour. 2026-08-18, the workspace merge. Feature-detected
   like everything above: no .jbNav or a jbShrunk page binds nothing. The bar's
   own wiring (menus, ink, observer) is untouched -- this only decides WHERE
   the same component lives, and remembers the choice. */
(function(){
"use strict";
var root=document.documentElement;
if(root.className.indexOf("jbShrunk")!==-1) return;
var nav=document.querySelector(".jbNav");
if(!nav) return;
root.className+=" jbRail";
try{ if(localStorage.getItem("jbRail")==="closed") root.className+=" jbRailClosed"; }catch(e){}

function setClosed(closed){
  var cls=root.className.replace(/\s*jbRailClosed/g,"");
  root.className=closed?cls+" jbRailClosed":cls;
  try{ localStorage.setItem("jbRail",closed?"closed":"open"); }catch(e){}
}
/* collapse: a quiet chevron beside the lockup, same ink box as a nav item */
var grpL=nav.querySelector(".jbGrpL");
if(grpL){
  var col=document.createElement("button");
  col.type="button"; col.setAttribute("aria-label","Collapse sidebar");
  col.className="ctl ctl--nav jbRailCollapse";
  col.style.marginLeft="auto";
  col.innerHTML='<svg class="gIco uiIcon" viewBox="0 0 24 24" aria-hidden="true" focusable="false"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M9 3v18"/></svg>';
  col.addEventListener("click",function(){ setClosed(true); });
  grpL.appendChild(col);
}
/* expand: the floating control when the rail is away */
var exp=document.createElement("button");
exp.type="button"; exp.setAttribute("aria-label","Expand sidebar");
exp.className="jbRailExpand";
exp.innerHTML='<svg class="gIco uiIcon" viewBox="0 0 24 24" aria-hidden="true" focusable="false" style="width:18px;height:18px"><rect width="18" height="18" x="3" y="3" rx="2"/><path d="M9 3v18"/></svg>';
exp.addEventListener("click",function(){ setClosed(false); });
