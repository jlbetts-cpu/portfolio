...
"}";
  function makeRenderer(cv,fixed,alphaOn){
    ...
    if(!gl){abort(new Error("WebGL context unavailable"));return null;}
    ...
    function setup(){
      ...
      gl.linkProgram(prog);if(!gl.getProgramParameter(prog,gl.LINK_STATUS))throw new Error("WebGL program link failed: "+gl.getProgramInfoLog(prog));gl.useProgram(prog);
