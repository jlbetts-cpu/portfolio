# Developmental Improvisation — build prompt v12 (as built: Sen, the headline that holds the photographs, flat colour, dark mode)

> **v12, 2026-09-05 — Jayden's sixth review, applied; supersedes every earlier note on gradients, the hero, the typeface and the footer.** (1) **Sen** (800 display, 700 titles) with Plus Jakarta Sans 400/600 — one variable latin file, 18KB; Instrument Serif is out. (2) **The hero is one headline**, with the four best photographs set into the sentence as shapes on the line (capsule, circle, rounded rectangle, squircle), the tagline and one button; the chips, the second button and the row of four photographs are gone. (3) **No gradients anywhere**: the band at the top and foot and every card bloom are removed; a hue is now a **flat tint** mixed into the raised ground at 22% (light) / 30% (dark), on the stacked cards, the testimonials, the newsletter, the popup and the footer. (4) **Dark mode is back** as a toggle in the header, applied before first paint and stored; the `contrast` gate now walks every text node in both themes and has a `--self-test`. (5) **The footer is one coloured module** inset from the page edges. (6) **The gallery runs edge to edge.** (7) **The star is gone** from the section labels, and with it the Welcome and Contact eyebrows, the card numbers, the "Email or Call Here:" line, the Home link, the footer's tagline, its Menu column and its four dead social links. (8) The stacked cards carry **one** photograph each, in one shape, sized by height. Reference: Josha.io, which sets photographs into its headline.

> **v11, 2026-09-05 — Jayden's fifth review, applied.** **Instrument Serif** for the display and every section title (his "C"); Plus Jakarta Sans for the rest. The hero is the band (now quiet: hues at 44% over white, 70%), the serif title, the tagline, the four chips, the two buttons, and **the four best photographs** on the column; the strip is its own **Gallery** section after the stack. **The logo is plain** in the header (no disc). The stacked cards are white with **one photograph rule** (big rounded square at the outer bottom corner bleeding past the edge, small circle over its inner top corner, mirrored on even cards); their blooms are gone and the remaining blooms are quieter (62% at most). **The band returns under the footer.** The five principles measured against, and where the page failed them, are in `di-site/docs/BUILD-LOG.md` (v6).

> **v10, 2026-09-05 — Jayden's fourth review, applied.** The hero is the Maeve reference he attached: **a soft band of the seven hues across the top of the page**, blurred and grained, fading into the ground, its colours drifting one pass every 72s (still under reduced motion); **the title centred** and sinking into the fade; the four skill chips from the copy as the only other colour (each hue at 40% over white); the two buttons; then the Gallery row (star label, two arrows) and the strip. **Every photograph opens in a lightbox**: a native dialog on the ink scrim, one photograph from a new 1440px set, arrows, keys and swipe through the whole page's set, Esc and the scrim to close, focus returning to the photograph; a drag on the strip never opens it. Gates: the header's links over the band at three moments of the drift, and a `lightbox` gate. "Constraint is the best design": the eyebrow and the left/right hero split are gone. **Mid-review notes, also applied:** the band's hues at 66% over white and spread across five page-widths (two or three at a time, not the wheel); the colour logo back in the header on a white disc with a halo of light; the stacked cards' bloom following how much of the card is on screen; the stacked cards' photographs bigger, placed free on a stage with the big one bleeding past the card's edge; Sen rendered against Plus Jakarta Sans and two serifs (`di-site/docs/type-specimen.jpg`) and not applied.

> **v9, 2026-09-05 — Jayden's third review, applied; supersedes every earlier note on themes, coloured surfaces, shapes and the logo.** (1) **The logo does not move.** (2) **One theme, light**: no dark mode, no toggle; the header is transparent on the ground and glass once scrolled, and **the mark in it is black**. (3) **Colour is contained**: it lives in the photographs, in the 16px section star, and in **the bloom**: a soft eased radial gradient at the foot of a white card (the hue mixed toward white in oklab over seven stops, its partner hue as a fainter second light, stops ending in transparent white). The bloom is always on for the **testimonials** (his reference: three white cards on a grid, the middle one lower), the newsletter card and the popup, and rises under the pointer on the stacked cards. No flat coloured surfaces, no coloured chips, no hover fills. (4) **The quote ring stays** and its shaped-photograph language extends to the stacked cards' tiles (a rounded square beside a circle). (5) **The decorative vectors are removed.** The gradient technique was learned from his own portfolio (`hero-time.css`, `footer.css`); the articles were blocked from the sandbox.

> **v8, 2026-09-05 — Jayden's second review, applied; supersedes v7 and any line below about the arch or frames.** (1) **No outlines** on photographs. (2) **The hero is the tennis-school reference** he attached: eyebrow with the star, a big left title, the tagline and buttons right, the sky arc and yellow star above them, two round arrows, then **the strip**: twelve 4:5 photographs on one looping track, moved by the flow (6px per degree: 22px/s at rest, faster with the scroll), one card per arrow press, draggable. Four options were weighed and are in `di-site/docs/BUILD-LOG.md`. (3) **The circle lives around the quote**: eight shaped photographs (rounded square → 45° tilted square → circle, as in the video he sent) on a 300px ring around “Creativity in motion creates knowledge!”, upright, turning with the flow, stopping under the pointer. (4) **Colour sparingly, on hover**: the stacked cards and the testimonials are white at rest and take their hue under the pointer (`data-surface="hover"`); the newsletter stays the one permanent coloured surface. (5) **Three shapes with flow**, drawn from the logo — the star, the arc, the ring — in three placements (hero, testimonials, newsletter), moved by the scroll part of the flow only. brandappart.com is blocked from this sandbox; the video and the reference image were the sources.

> **v7, 2026-09-05 — Jayden's review of the built site, all applied; this note supersedes any line below that still says dark ground, `#181818`, pause control or figures band.** (1) **Light mode is the default**: ground `#F7F5F0` (a warm off-white, hue ≈ 80°, not a yellow cream), ink `#1B1916`; **dark mode** (`#131211` / `#F4F1EB`) is the visitor's choice from a **toggle in the header**, remembered; the **header is the same warm black in both themes** with the **colour logo** in it, and the logo is **out of the hero**. (2) The **monogram is re-centred**: the ring's inner circle is at (393, 451.7) in the file and the letters sat 18 up and 5 left of it; every logo file carries `translate(5 18)` on the `d` and star. (3) **No pause control.** The arch and the header logo's ring are driven by **the flow**: a 3.75°/s drift plus 0.06° per scrolled pixel, eased (0.32s); a photograph under the pointer eases the drift to a stop (reference: brandappart.com, everything goes with the scroll). (4) **Home, Gallery, Contact sit on the right** with the toggle and Subscribe. (5) **The figures band is deleted**, with its asset. (6) **Each arch photograph sits in a 3px frame of one brand hue** (the seven hues cycling), which also removes the pale fringe he saw on the rotated edges. (7) **The stack's motion**: cards stick 12px lower each and a covered card scales from its top edge by 4.5% per card above it, in step with the scroll; the rotateX recede is gone. (8) **The newsletter card and popup are the orange surface.** (9) On the light ground the small marks (stars, initials, pager) use a **deeper tone of each hue** (`--m-*`) so a 16px star reads at ≥ 3:1. (10) The hero copy hangs **24px above the fold** and the title is capped at 5.8% of the hero's height, so it fits inside the arch on a 720px screen; cards are .32r (213px at 1440). (11) Photographs: boy-fist replaces bow-ties-wall in the arch; linda-portrait (arch) and linda-laughing (tile 01) swap. Everything is in `di-site/`, gates in `di-site/tools/gates` (32 lines pass), numbers in `di-site/docs/BUILD-LOG.md`.

> **v6, 2026-09-05 — the site is built; this prompt is now its spec of record.** The home page lives in `di-site/` on this branch (`index.html`, `styleguide.html`, `css/`, `js/`, `tools/gates/`, `docs/`). Four changes from v5, all Jayden's: the arch's cards **lean with the arch** again (tilt 0.4 × the signed angle — the earlier "wrong orientation" was a bug, not the idea: a 0→360° sweep gave the left half tilts over 120°, and the keyframes now flip the tilt's sign while a card is hidden at the bottom); **bigger photographs** (fourteen slots, cards 0.32 r, 203px wide at 1440; on mobile 116px in a three-card fan); the **logo's ring of figures turns** at the same rate as the arch; and the card colours are the **seven logo hues**, not the pastels (violet takes white ink). Nine gates pass with numbers in `docs/BUILD-LOG.md`; `orbit.mjs --self-test` fails on an injected overlap as it should.
...
4. **Colour lives on surfaces, chips, the arch's frames and the logo. Body text is only ink or an ink tier.** No coloured headings, no coloured paragraphs, **no gradient anywhere** — not in the hero, not behind the logo, not on a button, not in the footer. The ground is flat: `#F7F5F0` in light, `#131211` in dark (v7); the header is `#1B1916` in both.
...
### 6.0 Nav (`.nav`) — v7: the ink surface in both themes; brand left; Home, Gallery, Contact, the theme toggle and Subscribe on the right
...
### 6.1 Hero (`#top`) — v8: the strip (the arch is gone; see the v8 note)
...
.orbit { --hero-h: clamp(640px, 100svh, 960px);                 /* the hero's height; 100vh alone breaks at 1920×1080 where the hero caps at 960 */
         --r:  min(48vw, calc((var(--hero-h) - 88px) / 1.29)); /* ring radius: 629 at 1440×900, 490 at 1280×720, 492 at 1024×768 */
         --cw: calc(var(--r) * .32);                            /* card width, 4:5 → 203×254 at 1440×900 — "bigger, the main focus" */
         --cx: 50%; --cy: calc(var(--hero-h) - var(--r) * .08); /* the ring's centre sits just above the fold: the bottom cards are cut by the edge and the copy clears the buttons on short screens */
         --k: .4;                                               /* tilt = .4 × the SIGNED angle from the top */
         --n: 14; }
```
- **Fourteen slots, 25.7° apart, leaning with the arch.** Slot `i` sits at `θ = i × 25.7° + φ` at `(cx + r·sin θ, cy − r·cos θ)`, tilted by `0.4 × s` where `s` is the angle measured from the top in **−180…180** — so a card at −60° leans −24° and its mirror at +60° leans +24°. In CSS the counter-rotation cannot follow a sawtooth, so `@keyframes orbit` goes 0% → 50% with the counter-rotation `−(1−k)·angle`, jumps at 50.0001% (the card is invisible at the bottom) to `−180° − 180°·k`, and reaches `−360°` at 100%. **The bug this fixes:** a single 0→360° sweep tilts the left half by 100°+ and reads as people falling over — that was every "wrong orientation" note from v3 to v5. The arc step is `2πr/14 ≈ 0.45r`; the card height is `0.4r`, so cards never touch (the `orbit` gate's card-on-card check is zero).
...
- **The fourteen photographs, one per slot, none repeated** (names as in `images/manifest.json`): yellow-trousers · bow-tie-chairs · circle-hands · blue-shirts · laugh-hat · conga-line · linda-laughing · bow-ties-wall · floor-game · three-teens · row-linked-arms · scene-handshake · linda-stage · three-men. Colour and B&W alternate; no two group shots sit side by side; the 2011 boy is out. Crops are `--pos` values on each card, chosen by looking at each at 203px.
...
- **Mobile (≤767px).** The arch becomes a fan across the top and the copy sits below it: `--r: 360px; --cw: 116px`, ring centre at `532px`, fully visible while `|θ| ≤ 42°`, gone by `56°` (three large cards showing — "the photos aren't very big or visible on mobile"); the copy block starts at `top: 300px` (logo 72px, display `clamp(1.625rem, 8vw, 2rem)`, buttons stacked full-width to 320px).
...
- **The logo's ring turns.** The colour mark is inlined; its six ring paths (three figures, three arcs) sit in a `<g class="logo__ring">` with `transform-origin: 393.1px 420.9px` (the ring's centre in the 787×842 viewBox) and rotate once per `--dur-orbit`, the same direction and rate as the arch — "the kids around it rotate the same as the photos going round". The monogram and the star stay still. It pauses with the arch.
...
### 6.2 Figures band (`.figures`) — DELETED in v7 (Jayden: "looks creepy and not for my taste")
...
### 6.4 Quote (`.ring`, accent pink) — v8: the circle of eight shaped photographs around the quote
...
### 6.6 Newsletter (`#newsletter`, accent orange, a full-strength surface since v7)
