...
6. Night has a portfolio-specific dark-mode exception: theme the actual visible header specimen with a translucent purple-indigo material and light controls while preserving its thin rim. Add a separate **viewport-width, opaque deep-indigo** field behind and outside the header/hero boxes—from the page top through the hero—matching the gradient's darkest outer stop (`#141e4b` / `#0d1738`) so the hero edge blends into it. Only the lower region fades back to the normal page background by the tabs. Night is the only state allowed to escape the content container. It must not look like a pale lavender wash or second head halo, create horizontal overflow, or change geometry. Other states retain the normal light header and contained hero gradient. Stripe itself has no spill.
