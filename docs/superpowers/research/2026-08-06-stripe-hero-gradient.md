...
2. Use one clean, true-circle radial form centered at the bottom. Its curved perimeter must remain readable and horizontally contained inside the hero rather than stretching into an edge-to-edge ellipse. Small low mesh nodes, discrete color blobs, random rays, filaments, contour lines, and the site's Gradient Maker aesthetic must not be visible in the result.
...
5. Off is a hard reset: stop the mesh and explicitly suppress its fallback, bloom, portrait cast, and the original `.heroAura`.
   During active time states, suppress the original `.heroAura` as well so the contained circular gradient remains the only light source behind the portrait.
