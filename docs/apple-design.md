# Apple Design — the reference this site is polished against

Jayden brought this in on 2026-08-12: "its apple design skill and it will help
us polish the site even more." It is a distillation of Apple's WWDC design talks
-- chiefly *Designing Fluid Interfaces* (2018), *The Details of UI Typography*
(2020) and *Principles of Great Design* -- translated to the web.

READ THIS BEFORE building or reviewing anything gesture-driven, anything that
animates, translucent chrome, or type. It is not a style guide to copy; it is
the reasoning to design against.

WHERE IT ALREADY AGREES WITH THIS SITE, so you do not "fix" what is right:
  - spring `linear()` curves already exist as --sp-bounce / --sp-pop / --sp-settle
  - the motion ladder (--dur-press 100 ... --dur-enter 500) is the duration set
  - press feedback is on :active, not on click
  - `prefers-reduced-motion` is honoured across the site and gated by contracts
  - Instrument Sans at two weights is a settled decision -- section 15's advice to
    prefer the system font does NOT apply here, and the typeface is not up for
    discussion. Its advice on SIZE-SPECIFIC tracking and leading does apply.

WHERE IT DISAGREES WITH A SETTLED DECISION, the decision wins. See
`.claude/skills/this-sites-design-rules/SKILL.md` and CLAUDE.md. In particular:
the soccer game's chaos is deliberate, the companion heads are the only thing on
this site that casts a shadow, and several hand-tuned motion values are
load-bearing and must not be flattened into any ladder.
